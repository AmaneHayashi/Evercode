...
def test_stationarity(data):
    ...
    if p_val >= 0.05:
        print(
            "Time series data is not stationary. Adfuller test pvalue={}".format(p_val))
    else:
...
def shapiro_normality_test(data):
    ...
    if p_value >= 0.05:
        print("Data follows normal distribution: X~N" +
              str((np.round(np.mean(data), 3), np.round(np.std(data), 3))))
        print("Shapiro test p_value={}".format(np.round(p_value, 3)))
    else:
        print(
            "Data failed shapiro normality test with p_value={}".format(
                np.round(
                    p_value,
                    3)))
...
def find_sarima_with_ar(models, aic_threshold, mape_threshold):
    ...
    for model in models:
        if model.aic < aic_threshold:
            model_order = "ar:" + str(model.model_orders['ar']) + " ma:" + str(model.model_orders['ma']) + " s_ar:" + str(
                int(model.model_orders['seasonal_ar'] / 12)) + " s_ma:" + str(int(model.model_orders['seasonal_ma'] / 12))
            models_order.append(model_order)
            models_aic.append(model.aic)
            train_forecasts = training_data.values[1:].reshape(
                training_size - 1) - model.resid[1:]
            train_mae = mean_absolute_error(training_data[1:], train_forecasts)
            trains_score.append(np.round(train_mae, 3))
            preds = np.exp(
                model.predict(
                    start=test_start_date,
                    end=practical_end_date,
                    dynamic=True,
                    typ='levels'))
    ...
    # 绘制新模型效果
    colors = [
        'g' if x < mape_threshold else 'b' for x in model_properties['test_score']]
    plt.figure(figsize=(20, 6))
    plt.bar(
        model_properties['model_order'],
        model_properties['test_score'],
        color=colors)
    plt.xlabel('Model')
    plt.ylabel('MAPE: %')
    ...
    for i, aic in enumerate(model_properties['aic']):
        plt.annotate(
            np.round(
                aic,
                0),
            (model_properties['model_order'][i],
             model_properties['test_score'][1]))
...
def mean_abs_pct_error(actual_values, forecast_values):
    ...
    for i in range(len(forecast_values)):
        err += np.abs(actual_values.values[i] -
                      forecast_values.values[i]) / actual_values.values[i]
...
def evaluate_model(model):
    ...
    # 评估最佳模型的残差数据是否符合正态分布
    shapiro_normality_test(model.resid[1:].drop(
        index=pd.to_datetime(differed_start_date)))
    # 观察最佳模型的残差数据的自相关性
...
def predict_by_model(model, predict_start_date,
                     predict_end_date, calculate_mape=True):
    model_preds = np.exp(
        model.predict(
        ...
        print(
            "MAE:{}".format(
                np.round(
                    ...
                    2)))
    # 绘制预测结果效果
    model_data = training_data.values[1:].reshape(
        training_size - 1) - model.resid[1:]
...
original_data = pd.read_csv(
    filepath_or_buffer='express_predict.csv',
    index_col='date',
    parse_dates=True)
...
# 绘制原始数据特征
plot_data_properties(
    practical_data,
    'monthly express data from 2014-01-01 to 2019-12-01')
...
# 绘制季节, S=Y/T
seasonal_component = training_data.div(decomposition.trend, axis=0)
plt.figure(figsize=(12, 4))
plt.plot(seasonal_component)
plt.scatter(
    x=pd.to_datetime('2015-11-01'),
    y=seasonal_component.loc['2015-11-01'],
    marker='D',
    s=50,
    color='r')
plt.scatter(
    x=pd.to_datetime('2016-02-01'),
    y=seasonal_component.loc['2016-02-01'],
    marker='D',
    s=50,
    color='g')
plt.title('S=Y/T')
...
# 绘制残差, R=Y/(S*T)
residual_component = training_data.div(
    (decomposition.trend * decomposition.seasonal), axis=0)
plt.figure(figsize=(12, 4))
...
plt.grid(which='both')
years = int(np.round(len(practical_data) / 12))
for i in range(years):
    index = practical_data.index[i * 12:(i + 1) * 12]
    plt.plot(practical_data.index[:12].month_name(),
             practical_data.loc[index].values)
    plt.text(
        y=practical_data.loc[index].values[11],
        x=11,
        s=practical_data.index.year.unique()[i])
plt.legend(practical_data.index.year.unique(), loc=0)
...
plot_data_properties(
    logged_diffed_data,
    'Log transformed and differenced data')
...
plot_data_properties(
    ...
    'Log transofrmed, diff=1 and seasonally differenced data')
...
best_model, models = find_best_sarima(
    train_data=log_transformed_data, p=range(3), q=range(3), P=range(3), Q=range(3))
